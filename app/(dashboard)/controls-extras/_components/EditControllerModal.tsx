...
export type EditableController = {
  id: string;
  title: string;
  description: string;
  price?: number;
...
interface EditControllerModalProps {
  ...
  controller: EditableController | null;
}
...
export function EditControllerModal({
  ...
  const labelPrefix = "Controller";
  const modalTitle = "Edit Controller";
  ...
  useEffect(() => {
    ...
    if (controller) {
      ...
      setPrice(
        controller.price !== undefined && controller.price !== null
          ? String(controller.price)
          : ""
      );
      setDiscount(
        controller.discount !== undefined && controller.discount !== null
          ? String(controller.discount)
          : ""
      );
      const incomingBadges = normalizeBadges(controller.badges);
  ...
  const updateControllerMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (!response.ok || hasExplicitFailure) {
        throw new Error(data?.message ?? "Failed to update controller.");
      }
    ...
    onError: (error) => {
      toast.error(
        error instanceof Error ? error.message : "Failed to update controller."
      );
    },
  });
  ...
  const handleSubmit = () => {
    ...
    updateControllerMutation.mutate(controller.id);
  };
          ...
          <div className="max-h-[92vh] overflow-y-auto px-6 pb-6 pt-6 sm:px-8">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <label
                  htmlFor="controller-edit-upload"
              ...
              <div className="grid grid-cols-1 gap-4 pt-2 sm:grid-cols-2">
                <Button
                  ...
                  disabled={updateControllerMutation.isPending}
                ...
                >
                  {updateControllerMutation.isPending ? "Saving..." : "Update"}
                </Button>
              </div>
            </div>
          </div>
