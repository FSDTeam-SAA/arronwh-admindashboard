...
export function EditReviewModal({
  ...
  const updateReviewMutation = useMutation({
    mutationFn: async () => {
      ...
      // Only send review text and rating when there's no video
      const hasVideo = !!videoFile || (!!videoPreviewUrl && !videoRemoved);
      if (!hasVideo) {
      ...
      if (!response.ok || hasExplicitFailure) {
        throw new Error(getApiErrorMessage(data, "Failed to update review."));
      }
      return data;
    },
    onSuccess: () => {
        ...
        <DialogContent className="max-h-[92vh] !w-[900px] !max-w-[600px] gap-0 overflow-hidden rounded-[16px] border-none bg-white p-0">
          <div className="px-6 pb-6 pt-7 sm:px-7">
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="mb-2 block text-[14px] font-medium text-[#2D3D4D]">Video</label>
                <Input
                  ref={videoInputRef}
                  type="file"
                  accept="video/*"
